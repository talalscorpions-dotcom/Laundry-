...
class AppState extends ChangeNotifier {
  ...
  void assignPickupDriver(String orderId, String driverId) {
    _mutate(orderId, (o) {
      ...
      // The moment the driver is assigned is, in effect, when they "accept"
      // the pickup job — the admin analytics screen measures driver pickup
      // time from here to markPickedUp below.
      o.pickupAssignedAt = DateTime.now();
  ...
  void assignDeliveryDriver(String orderId, String driverId) {
    _mutate(orderId, (o) {
      ...
      o.deliveryAssignedAt = DateTime.now();
  ...
  void markOutForDelivery(String orderId) {
    _mutate(orderId, (o) {
      ...
      o.outForDeliveryAt = DateTime.now();
  ...
  /// Cancels an order. [category] and [cancelledByRole] are optional
  /// structured tags on top of the free-text [reason] — e.g. the customer
  /// tracking screen's "Cancel order" flow passes
  /// `category: CancellationReason.delay, cancelledByRole: 'customer'` so
  /// the admin analytics screen can report "cancellations due to delay"
  /// without parsing the activity log.
  void cancelOrder(String orderId, String reason, {CancellationReason? category, String? cancelledByRole}) {
    _mutate(orderId, (o) {
      o.status = OrderStatus.cancelled;
      o.cancelledAt = DateTime.now();
      o.cancellationReason = category;
      o.cancelledByRole = cancelledByRole;
  ...
  // ---- Admin analytics: reporting by period ----
  //
  // Everything below is filtered by a `ReportPeriod` (Today / This week /
  // This month / This year / All time) rather than being a fixed all-time
  // number like the getters above — this is what backs the admin
  // Analytics tab's period selector.

  /// Orders placed on or after [period]'s start (or every order, for
  /// [ReportPeriod.allTime]).
  List<LaundryOrder> ordersSince(ReportPeriod period) {
    final cutoff = period.startFrom(DateTime.now());
    if (cutoff == null) return orders;
    return orders.where((o) => !o.createdAt.isBefore(cutoff)).toList();
  }

  /// The order funnel for [period]: how many were received (placed) vs.
  /// actually picked up vs. delivered. `pickedUp` counts every order that
  /// reached pickup, regardless of where it is now (or was cancelled
  /// afterwards) — `delivered` only counts ones that made it all the way.
  int receivedCountFor(ReportPeriod period) => ordersSince(period).length;

  int pickedUpCountFor(ReportPeriod period) => ordersSince(period).where((o) => o.pickedUpAt != null).length;

  int deliveredCountFor(ReportPeriod period) =>
      ordersSince(period).where((o) => o.status == OrderStatus.delivered).length;

  /// Revenue generated by the app for [period] — GMV (what customers were
  /// charged) and the platform's commission cut of it. Cancelled orders are
  /// excluded, same as the all-time [totalGmv]/[totalCommission] getters.
  double revenueFor(ReportPeriod period) =>
      ordersSince(period).where((o) => o.status != OrderStatus.cancelled).fold(0.0, (s, o) => s + o.total);

  double commissionFor(ReportPeriod period) =>
      ordersSince(period).where((o) => o.status != OrderStatus.cancelled).fold(0.0, (s, o) => s + o.commissionAmount);

  /// How many orders each partner received in [period] — partnerId -> count.
  Map<String, int> orderCountByPartner(ReportPeriod period) {
    final counts = <String, int>{};
    for (final order in ordersSince(period)) {
      counts[order.partnerId] = (counts[order.partnerId] ?? 0) + 1;
    }
    return counts;
  }

  /// A pickup running longer than this counts as "slow" for the
  /// driver-performance report.
  static const Duration kSlowPickupThreshold = Duration(minutes: 15);

  List<Duration> _pickupDurationsFor(ReportPeriod period, {String? driverId}) {
    return ordersSince(period)
        .where((o) => o.pickupDuration != null && (driverId == null || o.pickupDriverId == driverId))
        .map((o) => o.pickupDuration!)
        .toList();
  }
  ...
  /// Average driver pickup time for [period] — measured from when a driver
  /// is assigned a pickup (`assignPickupDriver`, effectively "accepting" the
  /// job) to when they call `markPickedUp`. Pass [driverId] to scope it to
  /// one driver; omit it for the platform-wide average.
  Duration? averagePickupDuration(ReportPeriod period, {String? driverId}) =>
      _average(_pickupDurationsFor(period, driverId: driverId));

  /// How many of [driverId]'s pickups in [period] ran longer than
  /// [threshold] — omit [driverId] for the platform-wide count.
  int slowPickupCount(ReportPeriod period, {String? driverId, Duration threshold = kSlowPickupThreshold}) =>
      _pickupDurationsFor(period, driverId: driverId).where((d) => d > threshold).length;

  /// Per-driver pickup performance for [period] (drivers with zero pickups
  /// in the period are omitted) — what backs the "how many drivers took
  /// more time" report, broken down driver by driver.
  Map<String, DriverPickupStats> driverPickupStats(ReportPeriod period, {Duration threshold = kSlowPickupThreshold}) {
    final result = <String, DriverPickupStats>{};
    for (final driver in drivers) {
      final durations = _pickupDurationsFor(period, driverId: driver.id);
      if (durations.isEmpty) continue;
      result[driver.id] = DriverPickupStats(
        pickupCount: durations.length,
        average: _average(durations)!,
        slowCount: durations.where((d) => d > threshold).length,
      );
    }
    return result;
  }

  /// How many orders were cancelled in [period], and how many of those were
  /// specifically because the customer said the order was taking too long
  /// (`CancellationReason.delay`) — see `cancelOrder`.
  int cancelledCountFor(ReportPeriod period) =>
      ordersSince(period).where((o) => o.status == OrderStatus.cancelled).length;

  int cancelledDueToDelayCountFor(ReportPeriod period) => ordersSince(period)
      .where((o) => o.status == OrderStatus.cancelled && o.cancellationReason == CancellationReason.delay)
      .length;
